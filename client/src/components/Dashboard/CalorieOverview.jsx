        ...
        <div className="flex items-center space-x-4 p-4 rounded-2xl bg-slate-50 border border-slate-100">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-slate-800">
              {targetCalories} <span className="text-sm font-semibold text-slate-500">kcal</span>
            </div>
            <span className="text-[11px] text-slate-400">Theo thể trạng cá nhân</span>
          </div>
        </div>

        {/* Metric 2: Consumed (Đã nạp) */}
        <div className="flex items-center space-x-4 p-4 rounded-2xl bg-amber-50/70 border border-amber-100">
          ...
          <div>
            ...
            <div className="text-2xl font-black text-amber-900">
              {consumedCalories} <span className="text-sm font-semibold text-amber-700">kcal</span>
            </div>
          ...
          <div>
            ...
            <div
              className={`text-2xl font-black ${
                isOver ? 'text-rose-900' : 'text-emerald-900'
              }`}
            >
              {Math.abs(remainingCalories)}{' '}
              <span className={`text-sm font-semibold ${isOver ? 'text-rose-700' : 'text-emerald-700'}`}>
                kcal
              </span>
            </div>
            <span className={`text-[11px] font-medium ${isOver ? 'text-rose-600' : 'text-emerald-600'}`}>
      ...
      <div className="mt-8 space-y-2">
        <div className="flex justify-between items-center text-xs font-bold">
          <div className="flex items-center space-x-2 text-slate-700">
            ...
            {isOver && (
              <span className="bg-rose-100 text-rose-800 text-[10px] px-2 py-0.5 rounded-full uppercase">
                Vượt mức {Math.abs(remainingCalories)} kcal
              </span>
            )}
            {isPerfect && (
              <span className="bg-emerald-100 text-emerald-800 text-[10px] px-2 py-0.5 rounded-full uppercase flex items-center space-x-1">
